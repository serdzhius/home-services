(() => {
	...
	async function loadBlogItems() {
		const response = await fetch("https://github.com/serdzhius/home-services/files/blog.json", {
			method: "GET"
		});
		if (response.ok) {
	...
	function buildBlogItem(item) {
		...
		if (item.tags) {
			...
			for (const tag in item.tags) blogItemTemplate += `<a class="item-blog__tag" href="${item.tags[tag]}">${tag}</a>`;
